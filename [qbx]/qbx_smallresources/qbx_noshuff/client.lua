...
local function shuffleSeat(self)
    if QBX.PlayerData.metadata.ishandcuffed then
        return TriggerEvent('mqs-notify:sendNotify', 'error', locale('error.is_handcuffed'), 3000)
    end

    if LocalPlayer.state.seatbelt then
        return TriggerEvent('mqs-notify:sendNotify', 'error', locale('error.is_fastened'), 3000)
    end
