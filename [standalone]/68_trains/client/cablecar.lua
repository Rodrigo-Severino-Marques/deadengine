...
local CABLE_CARS = {
    [0] = { -- Left track car
        ...
        speed = 10.0, -- Movement speed modifier, determines the speed of the car on the track
...
function GivePlayerOptionToJoinMyCablecar(cablecar, moving)
    ...
    if not cablecar.is_player_seated then
        ...
        local dist = #(pos - plypos)
        if dist < 3.0 then
            DrawCablecarText3D("[~b~E~w~] Entrar no teleférico", pos.x, pos.y, pos.z + 1.0)
        ...
        if not moving then
            DrawCablecarText3D("[~b~E~w~] Sair do teleférico", pos.x, pos.y, pos.z + 1.0)
            if IsControlJustPressed(0, 38) then
                cablecar.is_player_seated = false
                DetachEntity(ply, 0, 0)
            end
