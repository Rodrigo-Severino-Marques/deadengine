...
local function DisplayNotification(msg)
	if Config.UseNativeNotifiactions then
		...
		TriggerEvent("mqs-notify:sendNotify", 2, msg, 5000)
	end
end
...
function startStressThread()
	...
	-- Try QBX stress system first, fallback to QB-Core if needed
	if GetResourceState('qb-core') == 'started' then
		TriggerServerEvent('hud:server:RelieveStress', math.random(4, 8))
	else
		-- Generic stress reduction for QBX or other frameworks
		TriggerServerEvent('qb-sitanywhere:server:reduceStress', math.random(4, 8))
	end
	DoneStress = false
end
